...
export default function Activities () {
  ...
  const getTk = async () => {
    try {
      ...
      if(parsedUserToken !== null && parsedUserToken !== undefined) {
        const verifyTk = await api.post("http://localhost:3001/activities", {
          userID: parsedUserToken.userID,
          userToken: parsedUserToken.token,
        });
  ...
  const handleDelete = async (id) => {
    try {
        const deleteNote = await api.delete(`http://localhost:3001/de-ac/${id}`);
  ...
  const handleUpdate = async (id) => {
      try {
        const filter = newAc.map(val => val).find(val => val.id === id);
        ...
        if(title !== undefined) {
          const updateNote = await api.put("http://localhost:3001/up-ac", {
            title,
            body,
            priority,
            id: editId,
          ...
          toastAlert({icon: 'success', title: `${updateNote.data.message}`, timer: 2000});
        };
      } catch (err) {
        toastAlert({icon: 'error', title: `${err?.response?.data?.message}`, timer: 2000});
      };
  ...
  const handleCreate = async (data) => {
    try {
      ...
      if(title !== undefined) {
        const create = await api.post("http://localhost:3001/new-ac", {
          title,
          body,
          priority, 
          userID: parsedUserToken.userID,
  ...
  const verifyUser = useQuery(['verifyUser', editId, deleteId, wasSaved, themeValue], getTk, {
    refetchOnMount: true,
              ...
              <Box component='form' onSubmit={editId !== null ? (handleSubmit(handleUpdate)) : (handleSubmit(handleCreate))}>
                ...
                <FormControl style={{paddingLeft: 3}}>
                  ...
                  >
                    ...
                    <FormControlLabel value="Minimum" control={<Radio style={theme.theme === 'dark' ? {color: '#EEEEEE'} : null} checked={priorityVal === 'Minimum' ? true : false} {...register('priority')} required  />} label="Small" />
                  </RadioGroup>
                </FormControl>
